...
--			storyboard.gotoScene( "scenes/"..event.target.id, "zoomOutInFade", 400 )
	...
	------------------------------------------------
	-- *** Create the contact info disply***
	------------------------------------------------
	local bg = display.newRect( screenGroup, 0, 180, 320, 270 )
	bg:setFillColor( 245,245,245 )

	--load contact details text
	local path = system.pathForFile( "contactDetails.txt", system.ResourceDirectory )
	local file = io.open( path, "r" )
	local contactDetailsText
	if file then    -- nil if no file found
	    contactDetailsText = file:read( "*a" )
	    io.close( file )
	else
		contactDetailsText = "No contact details available"
	end
	

	--set text options
	local optionsContact = {
		--parent = mydisplaygroup,
		text = contactDetailsText,
		x = display.actualContentWidth/2,
		y = 315,
		width = 280,	--required for multiline and alignment
	...
	local contactDetails = display.newText( optionsContact );
	contactDetails:setTextColor( 150,150,150 )
	screenGroup:insert( contactDetails )
